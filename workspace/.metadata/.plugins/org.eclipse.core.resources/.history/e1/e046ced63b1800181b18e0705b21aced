...
public class Order {
	...
	public String fromCSV(int nLineNum, List<String> lstHeader, List<String> lstLine) {
		...
		for ( int nIndex = 0; nIndex < nLineSize ; nIndex++ ) {
			...
			if ( nIndex < nLineSize -1 && 0 == strItem.length() ) return "No field can be empty apart from OrderDate";
			...
			case 4:
				...
				if (!validateEmailAddress(strBuyerEmail)) return "Invalid email address";
				break;
			case 5:
				strAddress = strItem;
				break;
			...
			case 10:
				...
				if (!bValid) return "Status must be IN_STOCK or OUT_OF_STOCK";
				break;
			case 11:
				if ( 0 == strItem.length()) break;
				DateFormat dfFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH);
				try {
					datOrderDate = dfFormat.parse(strItem);
